...
const ImageTrail = ({
  ...
  const handleMouseMove = (e: React.MouseEvent) => {
    ...
    if (distance > threshold && trailItems?.length) {
      ...
      if (zIndices.current.length !== images.length) {
        zIndices.current = Array.from({ length: images.length }, (_, index) => index)
      }

      const N = images.length
      const current = currentId.current
      const currentImage = images.item(current)

      if (!currentImage || N === 0) return

      if (zIndexDirection === 'new-on-top') {
        // Shift others down, put current on top
        for (let i = 0; i < N; i++) {
          if (i !== current) {
            zIndices.current[i] = (zIndices.current[i] ?? i) - 1
          }
        }
      ...
      images.forEach((img, index) => {
        img.style.zIndex = String((zIndices.current[index] ?? index) + baseZIndex)
      })
      ...
      animate(
        ...
        {
          x: [
            cachedMousePos.current.x - currentImage.offsetWidth / 2,
            mousePos.x - currentImage.offsetWidth / 2,
          ],
          y: [
            cachedMousePos.current.y - currentImage.offsetHeight / 2,
            mousePos.y - currentImage.offsetHeight / 2,
          ],
          ...keyframes,
        },
        {
  ...
  return (
    <ElementTag
      ...
      {...props}
    >
      {Array.from({ length: repeatChildren }).map((_, repeatIndex) => (
        <React.Fragment key={repeatIndex}>{children}</React.Fragment>
      ))}
    </ElementTag>
  )
}
