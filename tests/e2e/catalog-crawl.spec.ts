...
test('every published catalog URL is canonical, indexable, bilingual and in the sitemap', async ({ request }) => {
  ...
  for (let offset = 0; offset < checks.length; offset += 12) {
    ...
    await Promise.all(batch.map(async ({ routeRecord, locale }) => {
      ...
      expect(html.includes(`rel="canonical" href="${SITE.url}${path}"`), `${path} should be canonical`).toBe(true)
      expect(html, `${path} should not be noindex`).not.toMatch(/<meta[^>]+name="robots"[^>]+noindex/i)
      expect(html.includes(`hrefLang="${counterpart}" href="${SITE.url}${counterpartPath}"`), `${path} should link to ${counterpart}`).toBe(true)
      expect(sitemap.includes(`${SITE.url}${path}`), `${path} should be represented in sitemap alternates`).toBe(true)
    }))
  }
})

test('reserved, noindex, and unknown dynamic routes obey publication rules', async ({ request }) => {
  const reserved = await request.get('/de/kundenbereich')
  expect(reserved.status()).toBe(404)

  const sitemap = await (await request.get('/sitemap.xml')).text()
  for (const path of ['/de/staging', '/de/seo']) {
    const response = await request.get(path)
