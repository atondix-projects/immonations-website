...
export default async function HtmlSitemapPage({ params }: { params: Promise<{ locale: string }> }) {
  ...
  const labels = locale === 'de'
    ? { eyebrow: 'Alle Seiten', title: 'Sitemap', lede: 'Alle veröffentlichten Inhalte von Immonation, geordnet nach Ausbaustufe.', phase: 'Bereich' }
    : { eyebrow: 'All pages', title: 'Sitemap', lede: 'All published Immonation content, grouped by rollout area.', phase: 'Section' }
      ...
      <div className="mx-auto w-full max-w-[1240px] px-6 py-16 lg:px-10">
        {[...groups.entries()].map(([phase, routes]) => (
          <section key={phase} className="border-border border-t py-10 first:border-t-0 first:pt-0">
            <h2 className="font-serif text-2xl font-semibold">{labels.phase} {phase}</h2>
            <ul className="mt-6 grid gap-x-8 gap-y-3 sm:grid-cols-2 lg:grid-cols-3">
              {routes.map((routeRecord) => (
                <li key={routeRecord.id}>
                  <a className="text-brand-700 hover:text-brand-800 text-sm font-medium underline-offset-4 hover:underline" href={`/${locale}${routeRecord.paths[locale as Locale]}`.replace(/\/$/, '')}>
